...
- `lib/api-spec/openapi.yaml` is the canonical contract for every public HTTP endpoint (`/healthz`, `/paywall/invoice`, `/paywall/status/:hash`, `/paywall/dev-pay/:hash`, `/paywall/recover`, `/ice-servers`, `/openapi.yaml`, `/asyncapi.yaml`).
- `lib/api-spec/asyncapi.yaml` is the sibling AsyncAPI 3.0 contract for the Socket.io signaling channel mounted at `/api/socket.io`.
- Both spec files are served live at `GET /api/openapi.yaml` and `GET /api/asyncapi.yaml` (`Content-Type: application/yaml`, `Cache-Control: public, max-age=3600`). Content is bundled into the server binary at build time via esbuild text loader — no filesystem access at runtime. It enumerates every emit/receive event (`create-room`, `join-room`, `leave-room`, `lock-room`/`unlock-room`, `set-knock-mode`, `approve-knock`/`deny-knock`/`cancel-knock`, `extend-room`, `destroy-room`, `relay-signal`, `peer-media-state`, `request-screen-share`, `screen-share-started`/`stopped`) and every server broadcast (`peer-joined`/`peer-left`, `room-locked`/`room-unlocked`/`room-extended`/`room-expired`/`room-destroyed`, `knock-request`/`knock-mode-changed`/`knock-approved`/`knock-denied`, `screen-share-granted`/`denied`/`state`), with payload shapes, ack-callback shapes, and a shared `SignalingErrorCode` enum (`ROOM_EXPIRED`, `ROOM_LOCKED`, `SLOT_OCCUPIED`, `SLOT_RESERVED`, etc.). Treat both spec files as the source of truth — the prose in `VOID_TECHNICAL_OVERVIEW.md` §3.5 is the companion summary.
- **Recovery codes (Task #117):** `/paywall/status/:hash` mints a 4-word BIP-39 recovery code alongside the JWT at first poll-confirm. Code is shown once on the PaywallModal PAID screen with explicit "this is your only chance" framing — never auto-persisted on the client (no localStorage). `POST /paywall/recover { code }` redeems the code, single-use, and returns a fresh JWT clamped to remaining seconds in the original paid window. 404 conflates "wrong code" and "already redeemed" by design; 410 = window expired; 400 = malformed input. Codes live in memory only and are GC'd on next mint. See `VOID_TECHNICAL_OVERVIEW.md` §4.6.
- When adding or changing a public HTTP route, update the spec in the same change, then re-run `pnpm --filter @workspace/api-spec run codegen` to refresh the generated TypeScript types (`lib/api-client-react`) and Zod validators (`lib/api-zod`). Generated files are checked in.
- When adding or changing a signaling event, update `asyncapi.yaml` in the same change, then re-run `pnpm --filter @workspace/api-spec run codegen`. The generator script (`lib/api-spec/generate-signaling-types.ts`) reads `asyncapi.yaml` and writes `lib/signaling-types/src/generated.ts`. The `@workspace/signaling-types` package (`lib/signaling-types`) re-exports all generated types and is consumed by `@workspace/api-server` and `@workspace/void-client`.
...
`docs/code-quirks-index.md` is a navigable map of the **load-bearing weirdness** in this codebase — non-obvious decisions across audio, WebRTC, Lightning, crypto, signaling/rooms, the void-client build chain, Express middleware ordering, and the shared agent protocol. Read it before "cleaning up" anything that looks odd.
...
**Gold Voyager** is a Brutalist concrete aesthetic with warm amber geometry.

### CSS Tokens (index.css :root)
- `--bg: #BEB3A2` — warm concrete
- `--surface: #A89E90` — slightly darker concrete
- `--fg: #1E1A14` — deep brown-black
- `--fg-dim: #5C5040` — medium warm brown
- `--red: #CC2200` — brick red
- `--teal: #0D9D8B` — cold accent
- `--gold: #E8A200` — bright amber (primary CTA color)
- `--burnt: #C85A00` — burnt orange

### Decorative Geometry (applied to LandingPage, StartScreen)
All geometry is absolutely positioned with `pointerEvents: none`:
- Amber slab: top-left anchor 260×230px, opacity 0.82
- Brown box: mostly off-screen upper-right, opacity 0.485
- Concrete slab: right edge 33px wide, opacity 0.35
- Gold triangle: lower-left clip-path, opacity 0.5
- Amber column stripe: left-mid 14px wide
- Red cross: horizontal slash from off-left + vertical full-height at x=112
- Ghost rect: pale gold upper-right, opacity 0.22
- Teal accent: 16×16 square below install box, zIndex 30
- Amber rule: full-width thin line, lower third

### Typography
- Body: JetBrains Mono
- Wordmark (VOID): system-ui weight 900
- border-radius: 0 everywhere

### Button Classes
- `.void-btn--gold` — amber fill on hover/active (primary CTA)
- `.void-btn--teal` — teal fill on hover/active (join/secondary)
- `.void-btn--red` — brick red fill on hover/active (leave/danger)
...
- Wired into CI as an additional step on `.github/workflows/asyncapi-spec-drift.yml` (alongside `check-asyncapi`) and into the local `marketing-voice` validation workflow in `.replit` (alongside `check:phrases`, `check:literals`, `check:feature-policy-sync`, `check:og-routes`). The workflow's path-trigger list also includes `VOID_TECHNICAL_OVERVIEW.md` and the new script + `artifacts/void-client/package.json` so doc-only edits to §6.2 still get checked.
