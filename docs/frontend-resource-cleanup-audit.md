...
| 6 | ~~`pages/StillPoster.tsx` `buildPeerStream`~~ | ~~offscreen 2D canvas + `captureStream(15)` MediaStream per peer~~ | — | **File removed (Task #1125).** The still-poster page and its regen pipeline were deleted when the social card became a hand-chosen screenshot. Row retained for audit history; the finding was OK at audit time. |
