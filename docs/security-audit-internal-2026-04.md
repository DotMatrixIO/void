...
| M-06 | Medium | ✓ CLOSED | Low — a Dockerfile edit that moves `USER node` before a `COPY` layer that runs `pnpm install` as root would silently re-elevate; CI guard catches this | n/a (CI/code) | Both Dockerfiles ran as root | `Dockerfile`, `lib/void-agent-sdk/Dockerfile.pilot` |
...
- `signHello()` signs the canonical body with `SIGNING_CONTEXTS.HELLO` domain separation (from `@workspace/agent-protocol`) using `crypto.subtle.sign("Ed25519", …)`.
...
**Fix reviewed (`Dockerfile`, `lib/void-agent-sdk/Dockerfile.pilot`, `.github/workflows/docker-build.yml`, `.github/workflows/docker-pilot-build.yml`):**

Both production and pilot Dockerfiles now contain `USER node` as the final privilege directive before `CMD`/`ENTRYPOINT`. The pilot image additionally includes `RUN chown -R node:node /app` immediately before the `USER node` line to handle root-owned pnpm workspace files.

Two CI GitHub Actions workflows provide regression guards:
...
- `docker-pilot-build.yml` provides the equivalent guard for the pilot image.

Both workflows fail the build and surface an `::error::` annotation if the runtime user is anything other than `node`.
...
Reviewed `.github/workflows/docker-build.yml` and `.github/workflows/docker-pilot-build.yml`. Both workflows:
- Trigger on push and pull-request to `main` when any relevant file changes.
- Build the respective Docker image.
...
- The workflow comments explicitly cite `docs/security-audit-internal-2026-04.md §7.1` as the justification.
...
The replacement is a TypeScript brand: `Brand<"Secret">` and the `Secret<T>` alias defined in `lib/agent-protocol/src/brand.ts`, plus `markSecret()` / `unwrapSecret()` constructors. Every secret value in the workspace is now branded at its declaration site, and the brand is threaded through call signatures so that any downstream consumer also sees the `Secret<T>` type. The branded surfaces are:
...
| `parseAgentSecret()` return | `lib/void-agent-sdk/src/crypto.ts` | `Secret<Uint8Array>` |
| `generateAgentSecret()` return | `lib/void-agent-sdk/src/crypto.ts` | `Secret<string>` |
...
**All eight original findings (H-01, H-05, M-01–M-06) are confirmed closed** in code. The fixes are structurally sound and implemented consistently across all relevant call sites. Regression guards (CI non-root checks for both production and pilot images per `.github/workflows/docker-build.yml` and `.github/workflows/docker-pilot-build.yml`, static `pushState` test, GC on consumed-token maps) are in place.
...
| M-06 | Medium | Dockerfile runs as root — *fixed in task #173 (production image) and #193 (pilot image)* | `Dockerfile`, `lib/void-agent-sdk/Dockerfile.pilot` |
...
>
> - **Library.** `hash-wasm` 4.12.0, used in all three surfaces (browser `void-client`, Node API server fixtures via `agent-spike`, agent SDK `void-agent-sdk`). One library = one set of bindings = no risk of one runtime running argon2i while another runs argon2id, or one rejecting an output the other accepts.
> - **Single canonical primitive.** `lib/agent-protocol/src/argon2.ts` exports `ARGON2ID_ROOM_PARAMS`, `ROOM_DERIVATION_SALT`, and `deriveRoomBytesArgon2id(normalizedPhrase)`. All three surfaces import and call this exact function — there is no per-surface implementation that could drift.
...
> - **Test vectors.** `lib/agent-protocol/src/crypto-compat.test.ts` Tests 1, 6, 18, and 19, and `lib/agent-spike/src/test-vectors.ts` Test 1, all assert the new argon2id-derived `roomId`/`e2eKey` byte strings for fixed phrases. Test 18 pins a `(phrase, salt, params) → bytes` constant against the underlying `hash-wasm` primitive so any library output drift surfaces immediately. Test 19 pins the production-parameter output and confirms the browser and Node `deriveRoomCredentials` paths produce bit-identical bytes.
...
> - **Limitation.** No real 2019-era Android benchmark was run from the dev container; the parameter floor was selected from the dev-container measurement (`m=64 MiB / t=3 / p=1` ran ~360–415 ms on a Linux x64 / Node 24 machine). Mobile devices typically run 2–4× slower, so ~1 s on the target device is the expected upper end. If subsequent on-device measurement shows budget headroom, raising to `m=128 MiB` is a single-constant change in `lib/agent-protocol/src/argon2.ts` (and a deliberate re-vector pass through the test files listed above).
>
...
`artifacts/void-client/src/lib/signalCrypto.ts` and `lib/void-agent-sdk/src/crypto.ts` use `crypto.subtle.deriveBits({name:"ECDH",public:remotePub}, privKey, 384)` followed by HKDF-SHA256 with a 32-byte all-zero salt and an info string per derivation. Public keys are exchanged as raw 96-byte buffers, base64url-encoded.
...
- **Operator-doc cross-reference (added by Task #263):** the auto-relay-only-on-`.onion` client behavior referenced in this section is what makes the optional `.onion` mirror deployment behave correctly without operator action. Operators considering a clearnet + `.onion` mirror should read `docs/onion-mirror-runbook.md`; the standalone Tor-only deployment switch remains owned by Task #253 (StartOS / Umbrel manifest review).
**Updated 2026-05-02 (Task #260):** when VOID is loaded over a Tor `.onion` origin, the client now defaults to relay-only at three layers: (a) `PreviewGate` pre-checks the host's relay-only toggle and gates disabling it behind a confirmation modal that names the IP-leak consequence; (b) joiners reaching the gate from a `.onion` origin fetch `/api/room-state/:code` and see an inline warning when the room they are about to enter was created with relay-only off; (c) `RoomPage` initialises `iceTransportPolicyRef` to `"relay"` whenever `isOnionOrigin()` is true, so every PeerConnection this client constructs — initial join, knock-approved, and reconnect paths — is built with `iceTransportPolicy: "relay"` regardless of the room's server-side `relayOnly` flag. The local enforcement deliberately overrides the room setting so a Tor user cannot accidentally leak their clearnet IP into a room that some prior host left non-relay-only. The room-wide RELAY ONLY badge is still driven by server signals; a separate teal `CONNECTED VIA TOR ONION` indicator is shown on `StartScreen` and `RoomPage` so the user can confirm at a glance that the local override is engaged. This narrows the §2.3 leak path further: an honest peer reaching VOID via the canonical `.onion` cannot be tricked into leaking even if every other defense fails, because the `.onion` hostname itself is the trigger and an attacker who could rewrite the hostname has already achieved a phishing primitive that defeats other guarantees first.
...
This section records the residual deliverables of task #257. The bulk of #257's planned scope (the `lib/secrets/` convention path, the brand-typed marker for secret values, and the CI guard against `===` on secret-imported identifiers) was overtaken by task #264 (R-9.12), which landed first and shipped a strictly stronger implementation: a real `Brand<"Secret">` type at `lib/agent-protocol/src/brand.ts`, declaration-site branding for every secret value in the workspace, and a type-aware ESLint rule (`@workspace/secrets/no-secret-equality`) that follows the brand through the type-checker rather than through a regex over import identifiers. The grep-based guard #257 contemplated would have been a strict subset of what R-9.12 already enforces, so it was not landed as part of #257; the convention-path package was likewise not landed because its only role would have been to seed the grep guard.

**What #257 did ship that is not duplicated by #264:**

- New shared helper at `lib/agent-protocol/src/timing-safe-string.ts` exporting `timingSafeStringCompare(a, b)`. Implementation: UTF-8 encode both inputs, pad to the longer length with zero bytes, run `crypto.timingSafeEqual` over the equal-length buffers, and fold the length match into the boolean result. Re-exported from `@workspace/agent-protocol` so any server module can pick it up with one import. Unit tests at `lib/agent-protocol/src/timing-safe-string.test.ts` cover empty strings, equal-length differences, length-fold (prefix-vs-superstring), and multi-byte UTF-8. The helper is the canonical safe-compare for any future case where two `Secret<string>` values must be compared per byte; today the only such path inside the secret boundary is `crypto.timingSafeEqual` on already-`unwrapSecret()`-ed buffers, so the helper has no in-tree caller yet — it exists so the next caller does not have to re-implement the pad-and-fold pattern.
...
Mitigations not present in the current code:
...
### 5.1 `void-secret:` invite entropy

`crypto.ts` `generateAgentSecret()` produces 32 random bytes from `crypto.randomBytes`, base64url-encoded, prefixed with `void-secret:`. 256 bits of entropy. Adequate against any brute force in the documented model. Note: the agent-secret path uses HKDF-SHA256 directly on the high-entropy secret (not the argon2id phrase pipeline) — there is no cost-stretch step because the input is already 256 bits, so the cost-stretch would be pointless against guessing.

- **Severity:** Informational (positive finding).

### 5.2 `relayOnly` for agent rooms — server-side enforcement

`agent.ts` line 467-468 sets `_policyRelayOnly = true` for `roomType === "agent"`, irrespective of any `relayOnly` flag the joiner passes. Then validates `validateRelayServers(iceServers)` and configures `iceTransportPolicy: "relay"` on the PeerConnection. Server-side: `socketHandlers.ts` accepts `relayOnly` and `roomType` in `create-room` and stores both on the room. The client (SDK) re-derives `relayOnly = true` for agent rooms regardless of what the server returned. **The SDK is correct.**

The server *also* stores the `roomType` and could enforce relay-only for agent rooms at the SDP/ICE level, but it does not — it forwards encrypted blobs without inspection. Since the SDK forces relay-only locally and the agent SDK is the only client expected for agent rooms, this is acceptable; a malicious agent peer who patched their own SDK could bypass relay-only on their *own* end (leak their own IP), but cannot trick the honest peer's PeerConnection (which independently sets `iceTransportPolicy: "relay"`).

- **Severity:** Informational. The defense relies on every agent client being well-behaved, which is a weaker assumption than server-enforced policy.

### 5.3 Hello binding (agent SDK only)

`agent.ts` `exchangeHellos` (lines 851-960) builds a `helloBody` containing the protocol version, identity, capabilities, room type, the local ECDH public key string and its fingerprint, a random nonce, and a timestamp. It signs with a freshly generated Ed25519 keypair (per-session). On receive, it verifies:
- Required fields present.
- Ed25519 signature valid.
- `remoteHello.hello.ecdhFingerprint === this.remoteEcdhFingerprint`.
- `remoteHello.hello.ecdhPublicKey === this.remoteEcdhPubKeyStr`.
- Major protocol version match.

This is a real binding: the hello commits the peer's ECDH public key under a signing key, and the receiver checks the committed value matches the ECDH key it negotiated. **An MITM who flipped the ECDH key during the relay phase would be caught here.** This also limits trust delegation: the Ed25519 key is fresh per session, so it does *not* serve as long-term identity, but it does serve as in-session identity.

This binding is *only* implemented in the agent SDK. The browser-to-browser path does not have it. See §1.3 / **M-01**.

- **Severity:** Informational (positive finding for agent SDK; the absence on the browser side is the **M-01** finding).

### 5.4 Per-channel size caps

`agent.ts` `enforceChannelLimit` checks `byteLen > limit` per channel name (`void.control`, `void.rpc`, `void.stream`). Limits live in `@workspace/agent-protocol` `CHANNEL_LIMITS`. Standard `>` comparison; no integer-overflow risk in JS for any realistic message size. Verified.

- **Severity:** Informational.

### 5.5 Extra-peer guard (agent rooms)

`setupExtraPeerGuard` rejects any `peer-joined` event for a peer ID different from the negotiated remote and tears the agent down. This blocks a third-peer injection at the agent-SDK level even if the server were compelled to push a forged peer event.

- **Severity:** Informational (positive finding).

### 5.6 Empty-room host-claim equally applies to agent rooms

The **M-02** finding (any phrase-holder can claim host on an empty-but-not-expired room) applies to agent rooms too. For the agent room model where peers are pseudonyms anyway, the practical impact is smaller — there is no "destroy room" primitive an agent typically uses — but the privilege still transfers.
...
### 6.3 Agent SDK (`lib/void-agent-sdk`)

- `node-datachannel` (0.32.2) — native WebRTC bindings used by the Node-side SDK to participate in WebRTC sessions. **This is the highest supply-chain risk in the tree.** Native modules execute arbitrary C++ at install time. The maintainer is small in number; security history is not large but the surface is. A compromised release would land arbitrary native code on every machine that runs the SDK. Declared as a runtime dep in `lib/void-agent-sdk/package.json` and `lib/agent-spike/package.json`; the `pnpm-workspace.yaml` `onlyBuiltDependencies: [..., node-datachannel, ...]` entry (line 40) is **not** a dead workspace-member declaration — it is the install-script allowlist required for the native build to run, and it correctly maps to the two packages above (verified 2026-05-02, task #266).
- `socket.io-client` — paired with the server.
- `@workspace/agent-protocol` — local package; not third-party.
...
- `node-datachannel` (SDK only; not standard self-host).
...
The agent-pilot image (`lib/void-agent-sdk/Dockerfile.pilot`) was explicitly out of scope for #173 and shipped running as root through 2026-04. Task #193 closed that gap: a `RUN chown -R node:node /app` immediately precedes a `USER node` directive, both placed after the final `COPY` of the entrypoint script and before `ENTRYPOINT`. The chown is required because the pilot stage runs `pnpm install` as root during build (so `/app/node_modules` and the workspace tree land root-owned), and the entrypoint then invokes `pnpm --filter ... run dev` and `pnpm --filter ... run demo` at runtime — which need read access to the workspace and write access to pnpm's per-user cache under `$HOME` (the `node` user's `/home/node` is created by the upstream `node:22-slim` image, so no extra `mkdir`/`chown` of a home directory is needed). Demo and SDK code under `lib/void-agent-sdk` was checked the same way as the production image (ripgrep for `writeFile`/`createWriteStream`/`appendFile`/`mkdir`/`mkdirSync`/`writeFileSync` across `lib/void-agent-sdk/src`, `lib/void-agent-sdk/demo`, `lib/agent-protocol/src`) and writes nothing under `/app` itself — the chown is purely defensive against a future demo or fixture that does. No live `docker build` / `docker run` validation was performed in the dev container because it has no Docker daemon (`/var/run/docker.sock` absent); the change is a textual mirror of the #173 pattern that was validated in that task. The live-validation gap is now closed in CI by `.github/workflows/docker-pilot-build.yml`, which builds the pilot image on every push/PR touching the relevant paths and asserts `docker run --rm --entrypoint id void-pilot:ci -un` returns `node` (matched by the production-image guard at `.github/workflows/docker-build.yml`). See R-9.10 for the full guard description.

- **Severity:** Medium → Fixed (production image #173, pilot image #193). Container escapes are out of model, but a process inside either container that is exploited (e.g. via a future signaling-handler bug or a malicious tool description routed through the agent SDK) no longer gains root inside the container.
...
- **Status: ✓ CLOSED (Task #265).** Every Lightning HTTP call (LNbits `createInvoice` / `checkPayment`, BTCPay `createInvoice` / payment-methods / `checkPayment`) now flows through a `lightningFetch()` wrapper in `artifacts/api-server/src/services/lightning.ts` that arms an `AbortController` for `LIGHTNING_FETCH_TIMEOUT_MS = 8_000` and rethrows `AbortError` as the typed `LightningBackendUnavailableError`. The two paywall routes (`/api/paywall/invoice` and `/api/paywall/status/:hash` in `routes/paywall.ts`) catch this typed error in dedicated `catch` blocks and return HTTP `503 { "error": "LIGHTNING_BACKEND_UNAVAILABLE" }`; the client (`artifacts/void-client/src/components/PaywallModal.tsx`) recognises the 503 in `requestInvoice` and surfaces a "PAYMENT SERVICE IS SLOW TO RESPOND. TRY AGAIN IN A MOMENT." string instead of spinning forever. Regression coverage: `artifacts/api-server/src/__tests__/lightning-timeout.test.ts` covers (a) the 503 mapping for both routes against a mocked `LightningBackendUnavailableError`, and (b) the live `AbortController` path against a hung global `fetch`, asserting the rejection lands within the 8 s window. Fail-fast over retries was an explicit design choice — a hung Lightning backend is an operator problem and the user gets a hard error rather than a multi-minute spinner.
...
Spot-checked `LandingPage`, `WhyPage`, `ComparePage`, `ThreatModelPage`, `BiometricPage`, `AgentModePage` for claims. Most claims are accurate or hedged. The threat-model page should be amended to:
...
9. **StartOS / Umbrel manifest review.** ~~`umbrel-app.yml` and `manifest.yaml` were not exhaustively reviewed for permission requests and exposed-port surface. Addressing this requires a platform-by-platform manifest review against each store's policy.~~ ~~**WATCH residual (2026-05-02):** `marketing-claims-audit.md` carries a forward-looking `Tor-by-default` claim on `ThreatModelPage` that the manifest does not back; the manifest advertises both `tor-config` and `lan-config` interfaces side-by-side. Either soften the claim or ship a Tor-only default before submission.~~ **Updated 2026-05-03 (Task #238):** the Tor-by-default residual is closed. `ThreatModelPage`, `manifest.yaml`, `umbrel-app.yml`, and `README-selfhost.md` §6c now use the tightened `.onion`-reachable wording with an explicit media-path caveat (WebRTC ICE candidates remain on the user's underlying network), and `artifacts/void-client/scripts/banned-phrases.mjs` flags any reintroduction of `Tor-by-default` / `Tor-routed` in the scanned scope (with a lookahead exclusion for the legitimate "Tor-routed wallet/node" recommendations). The `marketing-claims-audit.md` row moved from WATCH to TIGHTENED in the same commit. **Updated 2026-05-02 (Task #253):** the manifest review pass has now landed at `docs/manifest-review-2026-05.md`. Every field of both manifests is enumerated against the policy clause it satisfies; the `tor-config` / `lan-config` audit (§3) confirms both interface blocks target the same backend port (3000) with no second listener that could create a problematic overlap; the Tor-only deployment switch (§4) is documented as a manifest-edit (delete the `lan-config` block) plus a reserved `TOR_ONLY=1` env-var contract; the persistent-state declaration (§5), Coturn host-network-mode justification (§6), upgrade behavior (§7), and Coturn config workflow reference (§8) are each declared at the manifest layer. **Residual gap:** the `wrapper-repo` / `upstream-repo` / `support-site` / `marketing-site` (StartOS) and `repo` / `website` / `submission` / `support` / `icon` (Umbrel) URLs remain RFC 2606 `void.example` placeholders pending public-repository publication; submission to either store is blocked on those swaps. The placeholder state is itself the correct posture today (better than inheriting a possibly-wrong fork URL) but must be resolved before the `.s9pk` is built and uploaded. Re-eval triggers and the calendar backstop are recorded in `docs/manifest-review-2026-05.md` §11. **Updated 2026-06-11:** the residual placeholder-URL gap is **closed** — the canonical public repository was locked at `https://github.com/Void-PWA/void` (`docs/launch-decisions.md` §7) and the `void.example` URLs were swapped to canonical values across both manifests (zero `void.example` strings remain). The manifest-review cross-reference is superseded by `docs/manifest-review-2026-06.md`. The only remaining submission-readiness step is the operator action of building the bumped `.s9pk` and verifying it installs in a StartOS test instance.

10. **Client integrity at runtime.** ~~Subresource Integrity (SRI) is not in use; the bundled output is single-origin.~~ **Updated 2026-05-02 (task #243):** SRI is now in place on the entry bundle. The post-build script `artifacts/void-client/scripts/add-sri.mjs` rewrites every emitted HTML file in `dist/public/` (the SPA entry plus the per-route social-card files from `gen-og-pages.mjs`) so each `<script type="module">` and `<link rel="stylesheet">` carries an `integrity="sha384-…"` attribute computed from the on-disk asset bytes, plus an explicit `crossorigin="anonymous"`. ~~Vite's build does not emit `<link rel="modulepreload">` tags for code-split chunks at this bundle size, so dynamic-`import()`'d chunks (`QrScannerModal-*.js`, `qr-scanner-worker.min-*.js`) are **not** covered by an SRI-bearing preload tag. Whether the browser enforces integrity on `import()` for those chunks is browser-dependent and historically incomplete: Chromium and Firefox do not currently extend the entry's integrity to descendant module fetches without an explicit `integrity` import attribute, which Vite does not emit.~~ **Updated 2026-05-02 (task #258):** the build now emits `<link rel="modulepreload" href="…" integrity="sha384-…" crossorigin="anonymous">` tags for **every chunk reachable from any entry via the transitive dynamic-import closure** (walking both `imports` and `dynamicImports` from Vite's manifest, not just first-level lazy imports). The injection is done by `artifacts/void-client/scripts/add-modulepreload-sri.mjs`, which runs after `gen-og-pages.mjs` so per-route social-card HTMLs get the same modulepreload set as `index.html`. The `vite.config.ts` build was switched to `manifest: true` so the closure walker has a manifest to read. As of this commit the closure for the void-client is `{QrScannerModal-*.js, qr-scanner-worker.min-*.js}` — `QrScannerModal` is dynamic-imported by `StartScreen.tsx`, and `qr-scanner-worker.min.js` is dynamic-imported in turn by `QrScannerModal`. The `sri.test.ts` regression guard was extended with parameterised assertions over the cross product of (every emitted HTML, every closure chunk) and a parameterised tamper-test (append a byte to each lazy chunk in turn, confirm the recomputed sha384 no longer matches the modulepreload integrity, restore) so the test fails loudly if the manifest walker silently drops a chunk. **Engine-support honesty paragraph:** SRI on `<link rel="modulepreload">` is required by current spec but enforcement varies by engine. SRI on modulepreload is reliably enforced in current Chromium-based browsers. Enforcement on other engines (Safari, older Firefox) is inconsistent. For Chromium users, lazy-chunk tampering now requires also tampering the entry HTML to omit or alter the integrity tag — a much louder operation that the CSP report-uri endpoint (Task #252) would catch. For non-Chromium users, the residual risk is lower than before this change but not eliminated. CSP `script-src 'strict-dynamic' 'nonce-…'` would close more of the residual gap but requires per-request server-rendered nonces, which is a much larger change than SRI; if the Task #252 CSP work surfaces strict-dynamic + nonces as desirable, it will be filed as a separate task with explicit scoping rather than bundled here. There are no lazy chunks in the current build that are reachable only at runtime conditional and therefore impossible to enumerate from the manifest — the manifest walker is exhaustive for the current code shape. The threat model for this gap is narrow — an attacker who can swap a built chunk on the same origin can almost certainly also serve a tampered `index.html` that drops the entry SRI tag entirely; SRI on the entry plus a future strict CSP buys defense-in-depth, not a hard guarantee against a fully-compromised origin. **CSP vs SRI**: CSP (`script-src`, `style-src`) restricts which origins a script or stylesheet may be loaded *from*; SRI verifies that the bytes of the loaded asset match what was *built*. They are complementary — CSP without SRI does not catch in-place tampering on a permitted origin, and SRI without CSP does not constrain *which* origin can serve the asset. **Failure mode**: when SRI rejects a tampered entry script the browser refuses to execute it; task #243 originally accepted the resulting blank page as the correct default (a real SRI failure is either an attack or a deployment bug, both warranting operator attention rather than a polished UI). **Updated 2026-05-02 (task #249):** the inline diagnostic UI deferred from #243 has now shipped. `artifacts/void-client/index.html` carries a small inline `<script>` that registers a capture-phase `error` listener on `window` and, when an `error` event targets a `<script>` or `<link>` element that has an `integrity` attribute, renders a plain HTML/CSS overlay into `#root` (or a sibling `<div>` if `#root` is missing) explaining that the page failed an integrity check and is not safe to load. The bootstrap is inline (so it cannot itself be subject to SRI — an SRI'd diagnostic would be defeated by the same failure it is trying to report), uses no external fonts, images, or network, and is idempotent (a single show, regardless of how many tags fail). The trigger is the presence of an `integrity` attribute on the failing element, so a 404 on a non-SRI'd asset (e.g. an OG image) does not flash the overlay; both an SRI mismatch and a 404 on an SRI'd asset do, which is the right behaviour because both are operator-attention deployment problems. Manual verification: appending a byte to a built `assets/index-*.js` and reloading shows the overlay instead of a blank page. The original §243 decision (broken-page is acceptable) and the §249 add-on (broken-page **with** a one-line explanation is better) are both consistent with this audit's framing — the goal was never a slick error UI, only that the user not be left wondering why the page is blank. **Reproducibility caveat**: ~~`pnpm --filter @workspace/void-client run build` followed by a `diff -r` of two consecutive `dist/` outputs shows differing chunk filenames and integrity hashes; the `__VOICE_MASK_VERSION__: JSON.stringify(Date.now().toString(36))` define in `vite.config.ts` is one identified non-determinism source.~~ **Updated 2026-05-02 (task #248):** The `__VOICE_MASK_VERSION__` cache-buster is now derived from a SHA-256 of `public/voice-mask-processor.js` rather than `Date.now()`, so the value changes if and only if the worklet source changes. Two consecutive runs of `pnpm --filter @workspace/void-client run build` now produce a byte-identical `dist/public/` tree (same chunk filenames, same SRI hashes, same HTML), verified by `diff -r`. The integrity baseline shipped in the entry HTML is therefore a function of the source tree, not of build wall-clock time, and an operator who wants to compare a deployed asset against the committed source can rebuild the same commit and expect bit-identical SRI hashes. The audit still cannot verify what a specific deployed CDN or reverse proxy actually serves; that remains a per-deployment operator concern.

11. **Browser-level surfaces inherited from the runtime.** DNS lookups, the system clipboard, the Notification API, extension DOM access, `RTCPeerConnection.getStats()`, and managed-browser `getUserMedia` permission logging are all readable to actors on the user's own device (other extensions, MDM-installed monitoring, OS-level loggers). The static audit can enumerate them — this re-audit does so in §R-10 — but cannot mitigate them in code, because the mitigations are user-side configuration choices on the browser the user runs. They are recorded as documented limitations rather than as assessable findings. Canonical user-facing disclosure: `ThreatModelPage.tsx` "BROWSER-LEVEL SURFACES"; technical mirror: `docs/threat-model.md` §6.
...
- **Likely safe but not verified at runtime** — applies wherever the property is a runtime behavior the audit did not measure: the `proof/server-state` timing-equalization (§8.2 / limitation 8), Coturn's actual `denied-peer-ip` enforcement under load (limitation 7), StartOS/Umbrel manifest correctness (limitation 9), `pnpm audit` output on the live lockfile at audit time (limitation 4 — see the lockfile-complete CVE appendix below), and any property that depends on the deployed network around the host. Where a §1–§10 paragraph relies on one of these properties, the language is "documented as" or "claimed to be" rather than "is".

**Lockfile-complete CVE appendix (Task #255, 2026-05-02).** The follow-up addendum named under limitation 4 above now exists at `docs/security-audit-cve-appendix.md`. It enumerates every advisory `pnpm audit --json --audit-level=info` returns against the lockfile, including Lows and Informational, and cross-references each High/Critical row to the audit ledger row that addresses it (or marks it "no ledger entry" when not yet addressed). The appendix is regenerated by `scripts/regen-cve-appendix.mjs` and is **deterministic from the lockfile and the commit that last touched it** — the committed file embeds the lockfile sha256 plus the lockfile-touching commit hash (resolved with `git log -1 -- pnpm-lock.yaml`), and nothing else; in particular it does not embed `HEAD`, the current branch, or the wall clock, so unrelated commits never invalidate it. Any change to the lockfile that surfaces a new advisory either updates the file (caught locally on `pnpm install`) or fails CI on the next PR. The release path is `.github/workflows/cve-appendix-release.yml`: (1) on PRs and `main` pushes that touch the lockfile, the appendix, the parser, or the wrapper, the workflow runs `--check` and fails the build on drift, so the commit a maintainer chooses to tag already carries a current appendix as part of its tree; (2) on `v*` tag pushes, the workflow re-verifies `--check` against the tagged tree and uploads a tag-stamped copy of the appendix as an immutable GitHub Release asset (`gh release upload`) rather than rewriting the published tag. The wrapper reuses `scripts/lib/pnpm-audit-parser.mjs`, the same parser Task #254's monitoring workflow consumes, so the per-release static snapshot and the live monitoring path cannot drift on JSON-shape handling, severity classification, or pnpm-version pin. The day-to-day open list lives in the operator-private issue tracker driven by Task #254; the appendix file itself is the public, point-in-time complement to §R-9.11 above.

**Post-audit fixes (May 2, 2026, follow-up — task #254):** `.github/workflows/pnpm-audit.yml` runs `pnpm audit --json` on every push/PR to `main` (build-fail) and on a daily schedule (issue-creation + optional webhook), with `scripts/audit/parse-audit.mjs` enforcing re-evaluation dates on every entry in `scripts/audit/ignore-list.json`; the workflow file's header comment documents the Dependabot division of labour and pins pnpm to `10.26.1` so the JSON shape the parser depends on cannot drift silently. This addresses the limitation of point-in-time manual audits by providing continuous monitoring of the dependency tree.

---

## Self-host package version bump — 2026-05-02 (Task #259)

`manifest.yaml` (StartOS) and `umbrel-app.yml` (Umbrel) move from
`1.1.0` to `1.2.0`. Both files now enumerate the audit-tracked
changes that have shipped since the 1.1.0 release notes were
written, so a downstream packager and the Task #253 review pass
can see them at the manifest layer rather than having to read the
audit doc to discover them. The release-notes block on each
manifest cites: H-01 (TRUST_PROXY_HOPS rightmost-XFF), H-05
(single-use room-creation JWTs binding `paymentHash`), M-01
(signed-hello with `roomId` binding and loud secure-channel
teardown), M-02 (host-role bound to recorded `paymentHash`), M-03
(`replaceState` on every leave path with a static regression
guard), M-04 (10–60 s settlement→room-create jitter, residual
documented), M-05 (placeholder TURN/PAYWALL secret startup
guards), M-06 (non-root container with CI assertion), the
`algorithms: ["HS256"]` JWT-verify pin, the `BROWSER-LEVEL
SURFACES` disclosure on `ThreatModelPage`, and the SRI /
reproducible-build / CSP-HSTS browser-side hardening that this
audit pass covered in §R-9 and the late-April delta.

The manifest text was also corrected for env-var drift against
`docker-compose.yml` and `Dockerfile`: `LIGHTNING_BACKEND`,
`PAYWALL_SECRET`, the LNbits/BTCPay credential triplets,
`TURN_URL`, `TURN_SECRET`, `TURN_CREDENTIAL_TTL` (default 4500),
`STUN_URL`, `ENABLE_AGENT_ROOMS` (default 0), `TRUST_PROXY_HOPS`
(default 1), and `LOG_LEVEL` are now enumerated in the StartOS
`alerts.start` block and the Umbrel `releaseNotes` field. The
StartOS `health-checks.web-ui` probe moved from `/` (which loads
the full client bundle) to `/api/health`, the dedicated JSON
health endpoint added in `routes/health.ts`. The `tor-config`
block was kept (it documents the StartOS Tor hidden-service
interface, which is one of two interface options the operator
chooses) but the manifest does not claim Tor is the *default*
exposure path — that claim remains a WATCH item in
`docs/marketing-claims-audit.md` and the open follow-up to
confirm or soften it before this package is submitted.

Two manifest items are deliberately deferred to the Task #253
review pass rather than litigated here: (1) the `wrapper-repo` /
`upstream-repo` / `support-site` / `marketing-site` (StartOS) and
`repo` / `website` / `support` / `submission` / `icon` (Umbrel)
URLs are deliberate placeholders pointing at the reserved
`void.example` domain (RFC 2606). The user confirmed on 2026-05-02
that no public repository has been published yet, so there is no
canonical URL to swap in at this version bump; the placeholders
make the unset state obvious at the manifest layer (rather than
inheriting a possibly-wrong `nicobao/sife` URL from 1.1.0) and a
single-line comment on each manifest names Task #253 as the owner
of the swap once the public repo lands. (2) Per-store
permission-policy compliance is the explicit Task #253 charter
and is not assessed here. The docker image surface itself is
unchanged in this bump — manifest text only.
