...
# 6. Push to the public remote over HTTPS with a Personal Access Token (PAT).
#    (operator does this once verification is green). An SSH deploy key also
#    works, but a PAT avoids provisioning a key on the build host. Keep the
#    token OUT of the remote URL and out of git config: an inline
#    https://<TOKEN>@github.com/... URL persists the secret in
#    "$PUB/.git/config" AND the reflog. Supply it per-push via an env var and
#    an Authorization header instead. The PAT needs 'repo' scope (classic) or
#    Contents: read/write (fine-grained) on the target repo.
git remote add origin https://github.com/DotMatrixIO/void.git
: "${GITHUB_TOKEN:?export GITHUB_TOKEN to a GitHub PAT before pushing}"
...
#    e.g.  git -c http.extraHeader="Authorization: Basic $(printf 'x-access-token:%s' "$GITHUB_TOKEN" | base64 | tr -d '\n')" \
#            push --mirror https://github.com/<private-owner>/void-history.git
```
