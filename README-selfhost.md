...
One CSP detail matters for bare-metal operators: under `SERVE_STATIC=1` the server computes `script-src` sha256 hashes for the inline script(s) in the built client HTML **once at startup**. Docker installs never notice (a rebuilt image is a restarted container), but if you rebuild the void-client in place on bare metal, restart the API server afterwards or browsers will block the client's inline integrity-diagnostic script under the stale hash.
