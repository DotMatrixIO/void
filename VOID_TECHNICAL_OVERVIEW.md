...
The `/paywall/status/:hash` route is intentionally **idempotent after first paid observation**. The server tracks each invoice in an `invoiceStates` map; the first paid poll mints the JWT + recovery code and stamps `settled = { token, expiresAt, recoveryRevealed: true }` onto the entry. Every subsequent poll of the same hash returns the **same** token and `expiresAt`, and **omits** the recovery code. This is what prevents a host (or anyone with the payment hash) from extending their paid window — or downgrading `day` → `standard` — by re-polling the status endpoint. The invariant is covered by three dedicated tests.
...
No persistent user accounts — fully stateless.
